...
def _load_courses(conn):
    with conn.cursor() as cur:
        cur.execute("""
                ...
                LEFT JOIN LATERAL (
                    SELECT p.place_id, p.place_name_kr, p.category, p.sub_category, p.image_url,
                        p.open_time, p.close_time, p.address_kr,
                        CASE
    ...
    for (course_id, title, image_url, created_at, duration, distance, seq, point_place_name, lat, lng,
         place_id, place_name, place_image_url, open_time, close_time,
         category, sub_category, address, tag_type, tag_name) in rows:
        course = courses.setdefault(course_id, {
            ...
            "distance_m": distance,
            "points": {},
            # subcategory: 창유님 추천 점수용 / sub_category: 지영님 화면용 (추후 한쪽으로 통일)
            "tags": {key: set() for key in ('category', 'subcategory', 'sub_category', 'region', 'purpose', 'mood', 'companion')},
        })
        course["points"].setdefault(seq, {
        ...
        if sub_category:
            ...
            course["tags"]["sub_category"].add(sub_category)
        if tag_type and tag_name:
            key = tag_keys.get(str(tag_type).upper())
            if key:
                course["tags"][key].add(tag_name)
                if key == "subcategory":
                    course["tags"]["sub_category"].add(tag_name)
